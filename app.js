...
function setMode(mode){
  appMode=mode;localStorage.setItem("stacktestpro_mode",mode);
}
...
function fd(){
  const fuelEl=$("fuel");
  if(!fuelEl)return null;
  if(fuelEl.value==="custom") return num("customFd");
  const value=parseFloat(fuelEl.value);
  return Number.isFinite(value)&&value>0?value:null;
}

// EPA Method 19 dry-basis F-factor calculation for NOx as NO2.
// This path is intentionally independent of lb/hr, stack flow, and heat input.
function m19(ppm,o2){
  const Fd=fd();
  if(ppm==null||o2==null||Fd==null||ppm<0||o2<0||o2>=20.9)return null;
  const cd=ppm*46.01/(385.3*1e6); // lb NOx/dscf from ppmvd
  return cd*Fd*20.9/(20.9-o2);    // lb/MMBtu by Method 19
}

function noxCalc(){
  const tester=m19(num("testerNox"),num("testerO2"));

  // Client comparison is optional. Use a client-reported lb/MMBtu value when
  // supplied; otherwise calculate the client value from ppm/O2 by Method 19.
  let client=num("clientReported");
  if(client==null) client=m19(num("clientNox"),num("clientO2"));

  const diff=(tester!=null&&client!=null&&client!==0)
    ?Math.abs(tester-client)/Math.abs(client)*100:null;
  const limit=num("permitLimit");
  const margin=(tester!=null&&limit!=null&&limit>0)?(limit-tester)/limit*100:null;

  // Direct lb/hr ÷ heat input is retained only as an OPTIONAL comparison.
  // It is not an input dependency for the Method 19 result.
  ...
  if(tester==null){
    ...
    flag.textContent="Enter tester NOx ppmvd, dry O₂ %, and fuel/Fd. Heat input and lb/hr are not required for Method 19.";
  } else if(client==null){
    flag.className="status ok";
    flag.textContent=`Tester Method 19 result calculated: ${fmt(tester,4)} lb/MMBtu. Client data is optional for comparison.`;
    ...
    flag.textContent="Tester/client difference exceeds 10%. Investigate basis, O₂, fuel/Fd, averaging, moisture, and client data.";
  }
}
...
function renderPrepPage(){
  $("prepMethodChips").innerHTML=selectedMethods.map(c=>`<span class="chip">Method ${c}</span>`).join("");
  $("prepMethodCount").textContent=selectedMethods.length;
...
function renderMasterEquipment(){
  ...
  $("prepEquipCount").textContent=items.length;
...
function updatePrepReady(){
  ...
  $("prepEquipLoaded").textContent=loaded;
  const f=$("prepReadyFlag");
...
function calcCommonJobPrep(){
  ...
  $("jpAreaOut").textContent=fmt(area,2);$("jpAcfmOut").textContent=fmt(acfm,0);$("jpDscfmOut").textContent=fmt(dscfm,0);$("jpDryMwOut").textContent=fmt(dryMw,2);
...
["jpStackShape","jpDiameter","jpWidth","jpDepth","jpTemp","jpVelocity","jpAcfm","jpDscfm","jpBws","jpO2","jpCo2","jpBaro"].forEach(id=>$(id)?.addEventListener("input",()=>{calcJobPrep();save();}));
...
$("fieldModeBtn")?.addEventListener("click",()=>{setMode("field");applyMode();});
$("fullModeBtn")?.addEventListener("click",()=>{setMode("full");applyMode();});
...
// Explicitly recalculate Method 19 whenever its independent inputs change.
["testerNox","testerO2","clientNox","clientO2","clientReported","fuel","customFd","permitLimit","testerLbHr","heatInput"].forEach(id=>{
  $(id)?.addEventListener("input",()=>{noxCalc();save();});
  $(id)?.addEventListener("change",()=>{noxCalc();save();});
});
