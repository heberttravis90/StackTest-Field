  ...
  if(code==="1"){
    return `<div class="m1-prep-card">
      ...
      <div class="grid two">
        ...
        <label>Distance A — upstream disturbance (ft)<input data-prep-input="1:A" type="number" step="any"></label>
        <label>Distance B — downstream disturbance (ft)<input data-prep-input="1:B" type="number" step="any"></label>
      </div>
  ...
  selectedMethods.forEach(code=>{
    if(code==="1"){
      ...
      if(flag){
        ...
        else if(plan.alternative){flag.className="status warn";flag.textContent=`Below 0.5D upstream and/or 2D downstream. Alternative Method 1 site-selection procedure is required; planning minimum shown: ${plan.required} points.`;}
        else if(aDia>=2&&bDia>=8){flag.className="status ok";flag.textContent=`Preferred 8D/2D location met. Governing minimum: ${plan.required} traverse points.`;}
...
function method1Field(){
  ...
  if(plan.alternative){
    ...
    f.textContent=`Below the normal simplified minimum of 0.5D upstream and/or 2D downstream. Method 1 alternative site-selection procedure applies; planning minimum shown is ${plan.required} points if that alternative procedure is used and accepted.`;
  } else if(aDia>=2&&bDia>=8){
    f.className="status ok";
    f.textContent=`Preferred 8D downstream / 2D upstream location is met. Minimum ${plan.required} traverse points shown.`;
  } else {
    f.className="status warn";
