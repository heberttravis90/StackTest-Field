...
function loadJob(){const raw=localStorage.getItem(STORAGE_KEY),s=$("saveStatus");if(!raw){if(s){s.className="status warn";s.textContent="No saved job found."}return}try{const j=JSON.parse(raw);methodSelectionMemory=new Set(j.methods||[]);
